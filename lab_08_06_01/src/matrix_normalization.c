...
int equalize_matrices(int ***matrix_a, int ***matrix_b, int *rows_a, int *rows_b, int *cols_a, int *cols_b)
{
    ...
    while (!error && *rows_a < target_size)
    {
        error = add_row(matrix_a, rows_a, *cols_a);
    }
    while (!error && *cols_a < target_size)
    {
        error = add_column(matrix_a, *rows_a, cols_a);
    }

    while (!error && *rows_b < target_size)
    {
        error = add_row(matrix_b, rows_b, *cols_b);
    }
    while (!error && *cols_b < target_size)
    {
        error = add_column(matrix_b, *rows_b, cols_b);
    }
